...
const filteredCompletedLists = completedLists.filter((list) => {
    ...
    return (!date?.from || listDate >= date.from) && (!date?.to || listDate <= date.to);
  });

const totalPrice = filteredCompletedLists.reduce((acc, list) => acc + (list.price || 0), 0);
...
return (
    <main>
      ...
      <Tabs defaultValue={listsTabs[0].value} className='w-full'>
        ...
        <TabsContent value="Not Completed" className=' mt-3 flex flex-col gap-5  w-full text-light-1'>
          {todoLists.map((list) => (
             <ListCard
             key={list.id}
             id={list.id}
        ...
        <TabsContent value="Completed" className='mt-3 flex flex-col gap-5  w-full text-light-1'>
          ...
          <div className="px-4">
          <p className="text-subtle-medium text-gray-1"> Total price: {totalPrice}</p>
          </div>
        </div>
